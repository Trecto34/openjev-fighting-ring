...
class OpenJevCrossEncoder:
    def __init__(self, path: str, subfolder: str | None = None, device: str | None = None, dtype=torch.bfloat16,
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(path, dtype=dtype, low_cpu_mem_usage=True, **kw)
    ...
    def latents(self, pairs) -> np.ndarray:
        ...
        result = np.concatenate(out, 0)
        self.last_latents = result
        return result
    ...
    def predict(self, pairs) -> np.ndarray:
        ...
        latent_out = []
        for i in range(0, len(pairs), self.bs):
            pooled = self._pooled(self._encode(pairs[i:i + self.bs]))
            latent_out.append(pooled.float().cpu().numpy())
            logits = self.model.score(pooled).float()
            out.append(torch.softmax(logits, -1).cpu().numpy())
        self.last_latents = np.concatenate(latent_out, 0)
...
class _RankMLP(nn.Module):
    def __init__(self, d, hidden=256, dropout=0.05):
        super().__init__()
...
class TetrisRankHead:
    """Parameter-efficient RoBERTa fine-tuning for Tetris move ranking.

    The Vulkan server exposes the frozen RoBERTa pooled state, so this head trains on
    that state locally. It uses listwise soft targets from a search teacher rather than
    a single positive label, which preserves how much better one placement is than
    another and avoids treating every non-best move as equally bad.
    """
    def __init__(self, d: int, hidden: int = 256, dropout: float = 0.05,
                 temperature: float = 0.35, lr: float = 2e-3, wd: float = 1e-3,
                 epochs: int = 80, patience: int = 10, seed: int = 0,
                 device: str | None = None):
        self.cfg = dict(d=d, hidden=hidden, dropout=dropout, temperature=temperature,
                        lr=lr, wd=wd, epochs=epochs, patience=patience, seed=seed)
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        torch.manual_seed(seed)
        self.model = _RankMLP(d, hidden, dropout).to(self.device)
        self.mu = np.zeros((1, d), np.float32)
        self.sd = np.ones((1, d), np.float32)
    ...
    @staticmethod
    def _groups(qid):
        return [np.flatnonzero(qid == q) for q in np.unique(qid)]

    def _loss(self, logits, targets, qid):
        losses = []
        for idx in self._groups(qid):
            z = logits[idx]
            y = targets[idx]
            y = torch.softmax((y - y.max()) / self.cfg["temperature"], dim=0)
            losses.append(-(y * torch.log_softmax(z, dim=0)).sum())
        return torch.stack(losses).mean()

    def fit(self, X, target, qid, val_frac: float = 0.15):
        X = np.asarray(X, np.float32)
        target = np.asarray(target, np.float32)
        qid = np.asarray(qid)
        tr, va = grouped_split(qid, val_frac, self.cfg["seed"])
        self.mu = X[tr].mean(0, keepdims=True)
        self.sd = X[tr].std(0, keepdims=True) + 1e-6
        Xtr, ytr = self._t(X[tr]), torch.tensor(target[tr], dtype=torch.float32, device=self.device)
        Xva = self._t(X[va])
        opt = torch.optim.AdamW(self.model.parameters(), lr=self.cfg["lr"], weight_decay=self.cfg["wd"])
        train_qid = qid[tr]
        best, best_state, bad = -1.0, None, 0
        for _ in range(self.cfg["epochs"]):
            self.model.train()
            loss = self._loss(self.model(Xtr), ytr, train_qid)
            opt.zero_grad(); loss.backward(); opt.step()
            self.model.eval()
            with torch.no_grad():
                pred = self.model(Xva).cpu().numpy()
            # Calculate validation top-1 directly from the teacher values.
            hits = []
            for idx in self._groups(qid[va]):
                hits.append(int(pred[idx].argmax() == target[va][idx].argmax()))
    ...
    def predict(self, X) -> np.ndarray:
        """Run the small head without PyTorch dispatch overhead per game move."""
        if self._np_params is None:
            state = {k: v.detach().cpu().numpy() for k, v in self.model.state_dict().items()}
            self._np_params = state
        x = (np.asarray(X, np.float32) - self.mu) / self.sd
        x = x @ self._np_params["net.0.weight"].T + self._np_params["net.0.bias"]
        # LayerNorm (the only normalization in the head).
        mean = x.mean(axis=1, keepdims=True)
        var = x.var(axis=1, keepdims=True)
        x = (x - mean) / np.sqrt(var + 1e-5)
        x = x * self._np_params["net.1.weight"] + self._np_params["net.1.bias"]
        # tanh GELU is fast and matches the default PyTorch GELU approximation closely.
        x = 0.5 * x * (1.0 + np.tanh(np.sqrt(2.0 / np.pi) * (x + 0.044715 * x ** 3)))
        return (x @ self._np_params["net.4.weight"].T + self._np_params["net.4.bias"]).reshape(-1)
